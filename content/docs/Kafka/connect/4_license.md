...
> "Kafka가 포함된 AI 플랫폼을 SaaS 형태로 타 회사에 판매"

이 경우 Confluent의 판단 기준은 **"고객이 Confluent 소프트웨어의 실질적 기능에 접근하는가"** 다.

```
[AI 플랫폼 SaaS]
    │
    ├── 고객이 사용하는 것: AI 기능, API, 대시보드
    │       └── Kafka Connect는 내부 파이프라인으로만 동작
    │               → 고객이 Connect에 직접 접근 불가
    │               → 회색지대 (법무 검토 필요)
    │
    └── 고객이 Connect REST API를 직접 호출하거나
        커넥터를 직접 설정할 수 있음
...
**결론:**
- Kafka Connect가 **내부 파이프라인**으로만 쓰이고 고객이 인지·접근하지 않는다면 → 법적 회색지대
- Connect 설정·관리 기능을 고객에게 직접 노출한다면 → **라이선스 위반 리스크 존재**
- SaaS 제품에 들어가는 컴포넌트는 **Apache 2.0으로 완전히 클리어**하는 것이 안전
...
|--|------------|-------------------|
| **적합한 케이스** | 일반 INSERT 적재 (지금 우리 케이스) | CDC 이벤트(INSERT/UPDATE/DELETE) 처리 |
