...
flowchart LR
    ...
    P["&lt;&lt;Interface&gt;&gt;<br/>UpdateAccountStatePort<br/>account.application.port.out"]
...
```mermaid
flowchart LR
    S1["SendMoneyService<br/>account.application.service"]
    S2["RegisterAccountService<br/>account.application.service"]
    R["&lt;&lt;Interface&gt;&gt;<br/>AccountRepository<br/>account.application.port.out"]
    A["AccountPersistenceAdapter<br/>account.adapter.out.persistence"]
...
로버트 C. 마틴의 표현을 빌리면, 필요 없는 화물을 운반하는 무언가에 의존하면 예상하지 못했던 문제가 생길 수 있다.
...
flowchart LR
    ...
    L["&lt;&lt;Interface&gt;&gt;<br/>LoadAccountPort<br/>account.application.port.out"]
    U["&lt;&lt;Interface&gt;&gt;<br/>UpdateAccountStatePort<br/>account.application.port.out"]
    C["&lt;&lt;Interface&gt;&gt;<br/>CreateAccountPort<br/>account.application.port.out"]
...
flowchart LR
    S1["SendMoneyService<br/>account.application.service"]
    S2["RegisterUserService<br/>user.application.service"]

    L["LoadAccountPort<br/>account.application.port.out"]
    U["UpdateAccountStatePort<br/>account.application.port.out"]
    CU["CreateUserPort<br/>user.application.port.out"]

    AA["AccountPersistenceAdapter<br/>account.adapter.out.persistence"]
    UA["UserPersistenceAdapter<br/>user.adapter.out.persistence"]
...
```mermaid
flowchart LR
    AS["CloseAccountService<br/>account.application.service"]
    BP["&lt;&lt;Interface&gt;&gt;<br/>CreateFinalBillUseCase<br/>billing.application.port.in"]
    BS["CreateFinalBillService<br/>billing.application.service"]
    BA["BillingPersistenceAdapter<br/>billing.adapter.out.persistence"]
