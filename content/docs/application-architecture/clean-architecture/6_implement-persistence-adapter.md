...
`RegisterAccountService`는 `createAccount()`만 필요할 수 있다.
...
ex) 일부 port는 JPA로 구현하고, 성능이 중요한 조회 port는 SQL 기반 adapter로 구현할 수 있다.
...
>
> ex) `@Transactional`은 메서드 호출 전 transaction을 열고,
...
1. 좁은 port interface를 사용하면 port마다 다른 방식으로 구현할 수 있다.
   ...
   ex) `LoadAccountPort`는 SQL query adapter, `UpdateAccountStatePort`는 JPA adapter로 구현한다.

2. port 뒤에서 application이 모르게 다른 영속성 기술을 사용할 수 있다.

   ex) `LoadAccountPort` 구현체가 PostgreSQL만 쓰다가 Redis cache를 먼저 확인하도록 바뀌어도,
   application service는 여전히 `LoadAccountPort`만 호출한다.

3. port 명세만 지켜지면 영속성 계층 전체를 교체할 수 있다.

   ex) `AccountPersistenceAdapter`를 JPA에서 MyBatis나 JDBC 기반 구현으로 바꿔도,
   `LoadAccountPort`, `UpdateAccountStatePort` 계약이 유지되면 application service는 그대로다.
